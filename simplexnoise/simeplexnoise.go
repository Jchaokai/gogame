...
//fractal brownian motion 布朗运动
//通过一些参数将代表noise的值 改变
func fbm2(x, y, frequency, lacunatity, gain float32, octaves int) float32 {
	var sum float32
	amplitude := float32(1.0)
	for i := 0; i < octaves; i++ {
...
func makenoise(pixels []byte, frequency, lacunatity, gain float32, octaves int) {
	...
	for y := 0; y < winHeight; y++ {
		for x := 0; x < winWidth; x++ {
			// noise[i] = fbm2(float32(x), float32(y), frequency, lacunatity, gain, octaves)
			noise[i] = turbulence(float32(x), float32(y), frequency, lacunatity, gain, octaves)
			if noise[i] < min {
				min = noise[i]
			} else if noise[i] > max {
				max = noise[i]
...
func main() {
	...
	gain := float32(.2)
	lacunatity := float32(3.0)
	// --

	makenoise(pixels, frequency, lacunatity, gain, octaves)
	for {
		...
		if keyState[sdl.SCANCODE_L] != 0 {
			lacunatity = lacunatity + 0.1*float32(lacunatity)
			makenoise(pixels, frequency, lacunatity, gain, octaves)
		}
